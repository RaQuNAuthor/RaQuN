...
from eval.visualization.misc import get_real_name, get_real_dataset
...
def create_tabular_overview(methods: [], datasets: [], results_per_method: {}):
    ...
    # We want to track the max weight, precision, recall and min runtime
    precision_max = {}
    recall_max = {}
    fm_max = {}
    weight_max = {}
    ...
    for method in methods:
        ...
        for dataset in datasets:
            ...
            if dataset not in precision_max:
                precision_max[dataset] = 0
            if dataset not in recall_max:
                recall_max[dataset] = 0
            if dataset not in fm_max:
                fm_max[dataset] = 0
            ...
            try:
                ...
                prec, prec_min, prec_max = get_basic_stats(result.lst_precision)

                rec, rec_min, rec_max = get_basic_stats(result.lst_recall)

                f, f_min, f_max = get_basic_stats(result.lst_f_measure)

                # Determine the new max for the precision
                if prec > precision_max[dataset]:
                    precision_max[dataset] = prec
                # Determine the new max for the recall
                if rec > recall_max[dataset]:
                    recall_max[dataset] = rec
                # Determine the new max for the F-Measure
                if f > fm_max[dataset]:
                    fm_max[dataset] = f
...
def create_num_of_comp_overview(datasets: [], results_per_method: {}):
    ...
    method_headers += " & \\multicolumn{2}{c}{RaQuN Low Dimensional}"
    ...
    for dataset in datasets:
        if dataset == "PPU" or dataset == "Apo-Games":
            tabular += "\\hline\n"
        raqun_stats = results_per_method["RaQuN_Weight_SmallVec"]  # type: MethodStatistics
...
def create_model_stats_overview(dataset_dir, all_datasets):
    # Initialize the string that represents the tabular
    tabular = "\\begin{tabular}{lrrrrrrr}\n"
    # first we add the method names as headers
    tabular += "\\hline\n"
    tabular += " & & \\multicolumn{3}{c}{Elements} & \\multicolumn{3}{c}{Properties} \\\\\n"
    tabular += "& Models & Avg. & Max. & Min. & Avg. & Max. & Min. \\\\\n"
    tabular += "\\hline\n"
    tabular += "\\hline\n"

    for dataset in all_datasets:
        path_to_file = dataset_dir + "/" + dataset + ".csv"
        dataset = get_real_dataset(dataset)
        ...
        for line in lines:
            ...
            if prop_max < len(properties):
                prop_max = len(properties)
            if prop_min > len(properties):
                prop_min = len(properties)
        ...
        tabular += "{0:,.0f} ".format(numpy.max(number_of_elements_per_model)) + " & "
        tabular += "{0:,.0f} ".format(numpy.min(number_of_elements_per_model)) + " & "
        ...
        tabular += "{0:,.0f} ".format(prop_max) + " & "
        tabular += "{0:,.0f} ".format(prop_min) + " \\\\\n "
    tabular += "\\hline\n"
    tabular += "\\end{tabular}\n"
