...
public class TreeManagerTest {
    ...
    @Test
    public void initializationWithOneModelAndSmallVectorization() {
        RModel model = getSimpleModel("A");
        List<RModel> modelList = new ArrayList<>();
        modelList.add(model);

        TreeManager manager = new TreeManager(modelList, TreeManager.EVectorization.LOW_DIM);
        assert manager.getNumberOfInputModels() == 1;
        assert manager.getNumberOfElementsInTree() == 1;
        assert manager.getElementsInTree().size() == 1;
    ...
    public void initializationWithTwoModelsAndLargeVectorization() {
        ...
        TreeManager manager = new TreeManager(modelList, TreeManager.EVectorization.HIGH_DIM);
    ...
    @Test
    public void differentElementsAtSamePointWithLowDim() {
        RElement extraElement = new RElement("A", "ele4", "4",
                Arrays.asList("n_ele4", "prop2", "prop3", "zzz"));

        List<RModel> models = generateModels();
        RModel modelA = models.get(0);
        modelA.addElement(extraElement);

        TreeManager manager = new TreeManager(models, TreeManager.EVectorization.LOW_DIM);
        List<QueryResult> queryResults = manager.queryElementsOfKNearestPoints(extraElement, 1);
        assert queryResults.size() == 2;
        for (QueryResult result : queryResults) {
            assert Double.compare(result.getDistance(), 0.0d) == 0;

            RElement element = result.getElement();
            assert element.getUUID().equals("1") || element.getUUID().equals("4");
            assert element.getName().equals("ele2") || element.getName().equals("ele4");
        }
    }
